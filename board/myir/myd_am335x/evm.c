...
#define BL_BIT                 (1 << 2)
    ...
    */
    ...
    //__raw_writel(__raw_readl(GPIO0_SETDATAOUT) | BL_BIT, GPIO0_SETDATAOUT);
    __raw_writel(BL_BIT, GPIO0_CLEARDATAOUT);
}

/* JBO */
extern enable_boot_pin_mux(void);
void wait_boot_key(void)
{
...
#define BOOT_PIN_GPIO_BIT      (1 << 3)
    ...
    enable_boot_pin_mux();
    while (1) {
        boot_key_pressed = __raw_readl(GPIO_DATAIN) & BOOT_PIN_GPIO_BIT;
        //printf("boot key pressed: %x\n", __raw_readl(GPIO_DATAIN) );
        if (boot_key_pressed == 0)
            elapse += 1;
        else
            elapse = 0;
        udelay(100000);
